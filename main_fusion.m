...
dataset ={'Dataset_TNO'};
Fcell = {'l2-norm'}; 
R_gradcell = {'TV-max','2FTV-max','4FTV-max','8FTV-max'};% {'2FTV-max','4FTV-max','8FTV-max'};
R_lumicell = {'Z-max'}; 
Fraccell = {'AL2'}; 
num_model = 0; 
WT = table;
% ilam = [0 0.2 2 3]; % '2FTV-max 2 & 18 ','4FTV-max 3 & 20', '8FTV-max 0.2 19.5'
% ibeta = [0 18 19.5 20]; 
                ...
                for i4 = 1 : length(Fraccell)
                    ...
                    for i5 = 1 : length(ilam)
                        for i6 = 1 : length(ibeta)
                            ...
                            mask_al2_RD = [cs_1 0 0;0 cs0 0; 0 0 cs1]; % 4 direcion
                            mask_al2_LU = [cs1 0 0;0 cs0 0; 0 0 cs_1]; % 1 direcion
                            mask_al2_LD = [0 0 cs_1;0 cs0 0; cs1 0 0 ]; % 3 direcion
                            mask_al2_RU = [0 0 cs1;0 cs0 0; cs_1 0 0]; % 2 direcion
...
%                                 if isfolder(savepathUVZ)==0
%                                     mkdir(savepathUVZ);
%                                 end
%                                 imwrite([U,V,Z]/255,[savepathUVZ,num2str(im),'.tif']);
                                %% 指标
                                metri_name = {'pairs','LAM','BETA','Order','MIN','Qsi','QABF','VIFP','NMIN','avg','std','me','VIF','IFC','SSIM','FSIM','SEN','CEN','MSE'};
    ...
    WT = table(Pairs,LAM,Order,MIN,QY,QABF,VIFP,NMIN,AG,SD,ME,VIF,IFC,SSIM,FSIM,SEN,CEN,MSE);
